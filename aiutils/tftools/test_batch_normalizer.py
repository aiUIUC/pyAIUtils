...
def test_batch_normalize_construct_2():
    ...
    bn = BatchNormalizer(x, 1e-5, ema, "batch_norm1")
    assert(bn)
...
def test_batch_normalize_construct_4():
    ...
    x = tf.placeholder(tf.float32, input_shape)
    ema = tf.train.ExponentialMovingAverage(decay=.9)
    bn = BatchNormalizer(x, 1e-5, ema, "batch_norm2")
    assert(bn)
...
def test_batch_normalize_construct_3():
    with pytest.raises(ValueError):
        ...
        x = tf.placeholder(tf.float32, input_shape)
        ema = tf.train.ExponentialMovingAverage(decay=.9)
        bn = BatchNormalizer(x, 1e-5, ema, "batch_norm3")
...
def test_batch_normalize():
    ...
    x = tf.placeholder(tf.float32, input_shape)
    ema = tf.train.ExponentialMovingAverage(decay=.9)
    bn = BatchNormalizer(x, 1e-5, ema, "batch_norm")
